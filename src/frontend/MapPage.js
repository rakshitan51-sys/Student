...
// ================================
// ✅ NEW: Road Route Geometry via OSRM
// Returns array of [lat, lng] for Leaflet Polyline
// Uses overview=full&geometries=geojson for road-following shape
// ================================
async function fetchRoadRoute(lat1, lng1, lat2, lng2, retries = 2) {
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      const controller = new AbortController();
      ...
      const res = await fetch(
        `${OSRM_BASE}/${lng1},${lat1};${lng2},${lat2}?overview=full&geometries=geojson`,
        { signal: controller.signal }
      );
      clearTimeout(timeout);
...
export default function MapPage() {
  ...
  const [distMode, setDistMode]       = useState(null);
  ...
  const [busToStudentRoad, setBusToStudentRoad] = useState(null); // [lat,lng][] or null
  ...
  // ================================
  // ✅ NEW: Fetch road route for bus → college
  // whenever busPosition changes
  // ================================
  useEffect(() => {
    let active = true;
    const [lat, lng] = busPosition;

    fetchRoadRoute(lat, lng, COLLEGE_LAT, COLLEGE_LNG).then((coords) => {
      if (active && coords) setBusToCollegeRoad(coords);
    });
  ...
  // ================================
  // ✅ NEW: Fetch road route for bus → student
  // whenever busPosition or studentPosition changes
  // ================================
  useEffect(() => {
    if (!studentPosition) return;
    let active = true;
    const [busLat, busLng] = busPosition;
    const [stuLat, stuLng] = studentPosition;

    fetchRoadRoute(busLat, busLng, stuLat, stuLng).then((coords) => {
      if (active && coords) setBusToStudentRoad(coords);
    });
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ✅ CHANGED: Bus → Student road polyline (was straight red line) */}
        {studentPosition && busToStudentRoad && (
          <Polyline
            positions={busToStudentRoad}
            color="red"
            weight={4}
          />
        )}
        {/* Fallback straight line while road route is loading */}
        {studentPosition && !busToStudentRoad && (
          <Polyline
        ...
        {/* ✅ CHANGED: Bus → College road polyline (was straight blue line) */}
        {studentPosition && busToCollegeRoad && (
          <Polyline
            positions={busToCollegeRoad}
            color="blue"
            weight={4}
          />
        )}
        ...
        {studentPosition && !busToCollegeRoad && (
          <Polyline
            positions={[[COLLEGE_LAT, COLLEGE_LNG], busPosition]}
            color="blue"
            weight={4}
            dashArray="8 6"
            opacity={0.5}
